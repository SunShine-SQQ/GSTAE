...
x_train = X_set[:1000,:]
y_train = Y_set[:1000,:]
x_valid = X_set[1000:2000,:]
y_valid = Y_set[1000:2000,:]
x_test = X_set[2000:3000,:]
y_test = Y_set[2000:3000,:]
...
print('数据读取完成，训练集大小：%d, 验证集大小：%d, 测试集大小： %d' % (len(y_train),len(y_valid),len(y_test)))
...
weights = {
        ...
        'w1_dec': get_weight([n_h1, n_input], 0.01, 'cost_AE1'),
        'w1_out': get_weight([n_h1, n_output], 0.01, 'cost_AE1'),
        #第二个AE    
        'w2_enc': get_weight([n_h1, n_h2], 0.01, 'cost_AE2'),
        'w2_dec': get_weight([n_h2, n_h1], 0.01, 'cost_AE2'),
        'w2_out': get_weight([n_h2, n_output], 0.01, 'cost_AE2'),
        #第三个AE    
        'w3_enc': get_weight([n_h2, n_h3], 0.01, 'cost_AE3'),
        'w3_dec': get_weight([n_h3, n_h2], 0.01, 'cost_AE3'),
        'w3_out': get_weight([n_h3, n_output], 0.01, 'cost_AE3'),
        #门控输出
...
with tf.Session() as sess:
    ...
    print('第一次特征编码完成，h1_train.shape = (%d, %d)'\
          % (h1_train.shape[0], h1_train.shape[1] ))
    ###########################################################################
    print('开始训练第二个AE...')
    avg_cost2 = []
    for epoch in range(epochs):        
    ...
    print('第二次特征编码完成，h2_train.shape = (%d, %d)'\
          % (h2_train.shape[0], h2_train.shape[1]))
    ###########################################################################
    print('开始训练第三个AE...')
    avg_cost3 = []
    for epoch in range(epochs):        
    ...
    print('第三次特征编码完成，h3_train.shape = (%d, %d)'\
          % (h3_train.shape[0], h3_train.shape[1]))
    ###########################################################################
    ...
    np.save("./record/rmse%f, r2=%f"% (test_rmse,r2), [y_test, y_predict])
    np.save("./record/gate——rmse%f, r2=%f"% (test_rmse,r2), [G1, G2, G3])
    #输出值
    fig = plt.figure(5)
    plt.plot(y_test,'-ob')
